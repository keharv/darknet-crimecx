...
extern void detect_cat(char *datacfg, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen, network *net);
...
int main(int argc, char **argv)
{
    ...
    if(argc < 2){
        fprintf(stderr, "usage: %s <cfg> <weights>\n", argv[0]);
        return 0;
    }
    ...
    if ((dir = opendir ("images/")) != NULL) {
        while ((ent = readdir (dir)) != NULL) {
            if (strstr(ent->d_name, ".jpg") || strstr(ent->d_name, ".png")){
                ...
                detect_cat("cfg/coco.data", target_file, thresh, .5, outfile, fullscreen, net);
                free(target_file);
            }
        }
        closedir (dir);
