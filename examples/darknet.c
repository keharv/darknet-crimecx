...
extern void detect_cat(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen);
...
int main(int argc, char **argv)
{
    ...
    } else if (0 == strcmp(argv[1], "detectcat")){
        float thresh = find_float_arg(argc, argv, "-thresh", .5);
        char *filename = (argc > 4) ? argv[4]: 0;
        char *outfile = find_char_arg(argc, argv, "-out", 0);
        int fullscreen = find_arg(argc, argv, "-fullscreen");
        detect_cat("cfg/coco.data", argv[2], argv[3], filename, thresh, .5, outfile, fullscreen);
    } else if (0 == strcmp(argv[1], "detectcats_folder")){
        float thresh = find_float_arg(argc, argv, "-thresh", .5);
        char *filename = (argc > 4) ? argv[4]: 0;
        char *outfile = find_char_arg(argc, argv, "-out", 0);
        int fullscreen = find_arg(argc, argv, "-fullscreen");
        ...
        if ((dir = opendir ("images/")) != NULL) {
            while ((ent = readdir (dir)) != NULL) {
                if (strstr(ent->d_name, ".jpg") || strstr(ent->d_name, ".png")){
                    printf("target: %s\n", ent->d_name);
                    char *target_file = malloc(sizeof(char)*(strlen("images/")+strlen(ent->d_name)+2));
                    sprintf(target_file, "images/%s", ent->d_name);
                    detect_cat("cfg/coco.data", argv[2], argv[3], target_file, thresh, .5, outfile, fullscreen);
